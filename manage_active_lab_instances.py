...
def active_labs():
    ...
    labs = [
        Lab(
            ...
            _minutes_remaining=minutes_remaining(df.loc[i].at["runningStartTime"], df.loc[i].at["runningDurationMinutes"])) for i in range(rowCount)
    ]
...
async def active_lab_metrics(q: Q, row: int):
    ...
    if row == 0: 
        ...
        individual_lab_metrics = [
            ...
            ui.textbox(name='runningDurationMinutes', label='Minutes remaining', value=minutes_remaining(df.loc[row-1].at["runningStartTime"], df.loc[row-1].at["runningDurationMinutes"]), disabled=True),
...
def extend_lab_instance_method(lild: int):
    session_holder = session.get_session()
...
def minutes_remaining(running_start_time: int, running_duration_minutes: int):  
